...
### 날짜 범위 필터 — DatePicker 팝오버 (WF-03)

`[📅 기간: 전체 ▾]` 버튼 클릭 시 달력 팝오버로 시작일~종료일 범위를 지정합니다.

```
│  [카테고리 전체  ▾]  [📅 2026-05-28 ~ 05-29  ▾]             │
│                                                             │
│  ╭──────────────────────────────────────────────────────╮  │
│  │  날짜 범위 필터                             [초기화]   │  │
│  │                                                      │  │
...
│  │       [ 취소 ]      [──── 적용 ────]                 │  │
│  ╰──────────────────────────────────────────────────────╯  │
│  (* radius-lg, bg-secondary, spring 200ms 등장)            │
```

> 날짜 범위 필터 적용 시 `GET /todos?dueDateFrom=2026-05-28&dueDateTo=2026-05-29` 호출
...
### 날짜 선택 — DatePicker 팝업 (데스크톱)

날짜 필드 클릭 시 하단에 달력 팝오버(popover)가 등장합니다.
...
│         │  ╰─────────────────╯ ╰─────────────────╯ │            │
│         │  (* 클릭된 필드 System Blue border 강조)   │            │
│         │                                          │            │
│         │  ╭──────────────────────────────────────╮ │            │
│         │  │  ←          2026년 5월          →    │ │            │
│         │  │  (* headline 17pt Semibold, 중앙)     │ │            │
│         │  │ ──────────────────────────────────── │ │            │
│         │  │  일   월   화   수   목   금   토     │ │            │
...
│         │  │  (* [28]: 오늘 — 외곽선 원 표시)      │ │            │
│         │  ╰──────────────────────────────────────╯ │            │
│         │  (* radius-md 12px, bg-secondary 배경)    │            │
│         │  (* spring(0.4,0,0.2,1) 200ms 등장)       │            │
│         ╰──────────────────────────────────────────╯            │
```

### 날짜 범위 선택 상태 (데스크톱)

시작일 선택 후 종료일 선택 시, 범위가 파란 배경 bar로 표시됩니다.
...
│         │  │  24   25   26   27  ●28───29●  30   │ │            │
│         │  │                    ↑────────↑        │ │            │
│         │  │  (* ●: filled System Blue 원)        │ │            │
│         │  │  (* 28~29 사이: System Blue 10% 배경  │ │            │
│         │  │     bar — 반투명 범위 하이라이트)      │ │            │
│         │  │  31                                  │ │            │
│         │  ╰──────────────────────────────────────╯ │            │
```

### 날짜 선택 — DatePicker (모바일 Bottom Sheet 확장)

모바일에서는 날짜 필드 탭 시 Bottom Sheet가 달력 영역을 포함하여 확장됩니다.
...
| 상태 | 표현 | 색상 |
|------|------|------|
| 일반 날짜 | 숫자 (body 17pt) | text-primary |
| 오늘 | 숫자 + 외곽선 원 | color-blue (border only) |
| 선택된 날짜 (시작/종료) | ● 숫자 filled 원 | color-blue (filled) |
| 선택 범위 내 날짜 | 숫자 + 배경 bar | color-blue 10% opacity |
| 비활성 날짜 (이전 달) | 숫자 | text-secondary (opacity 0.3) |
| 선택 불가 (종료일 < 시작일) | 숫자 | color-red opacity 0.3 + strike |
...
| **DatePicker (데스크톱)** | **날짜 필드 클릭 → 캘린더 팝오버(radius-md, spring 200ms)**  |
| **DatePicker (모바일)** | **날짜 필드 탭 → Bottom Sheet 내 달력 영역 확장(spring 300ms)** |
| **날짜 범위 하이라이트** | **시작일~종료일 사이 System Blue 10% 배경 bar 표시**           |
...
| 날짜 필드 탭 (모바일) | Bottom Sheet 확장 + 달력 영역 spring 300ms 슬라이드다운, selection haptic   |
| 날짜 필드 클릭 (데스크톱) | 캘린더 팝오버 spring 200ms 등장, 외부 클릭 시 닫힘                      |
| 시작일 선택          | 종료일 필드 자동 포커스 이동 (범위 선택 유도), selection haptic               |
| 범위 선택 완료       | 달력 닫힘 + 두 필드에 선택 날짜 채움 + 범위 유효성 즉시 검증                 |
...
| WF-03 할 일 목록 | `GET /todos`, `GET /categories` | 완료 토글: `PATCH /todos/:id/complete` or `/incomplete`<br>삭제: `DELETE /todos/:id`<br>날짜 범위 필터: `GET /todos?dueDateFrom=&dueDateTo=` |
