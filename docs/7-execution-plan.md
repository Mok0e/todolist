...
> 병렬 실행 시 크리티컬 패스 기준 약 17시간 = 약 2개 작업일
...
크리티컬 패스(`:crit` 표시): **BE-01 → BE-03 → BE-04 → BE-05 → BE-06 → FE-05**

총 크리티컬 패스 소요: 1 + 2 + 3 + 2 + 4 + 5 = **17시간 (2개 작업일)**
...
| 05/29 (금) PM    | Phase 5   | FE-05                                           | 전체 완료                          |
...
- [ ] Floating Label 입력 컴포넌트 (포커스 시 레이블 위로 spring 애니메이션)
- [ ] Center Card 반응형 레이아웃 (≥768px 480px 고정폭, <768px 풀너비 + 16px 패딩)
